...
namespace TaskManagement.Controllers
{
    ...
    public class TaskController : ControllerBase
    {
        ...
        [HttpGet("GetOngoingTasksInRange")]
        public IActionResult GetOngoingTasksInRange(int userID, DateTime startDate, DateTime endDate)
        {
            var tasks = _taskRepository.GetOngoingTasksInRangeTime(userID, startDate, endDate);
            return Ok(tasks);
        }

        [HttpGet("GetOngoingTasksInDay")]
        public IActionResult GetOngoingTasksInDay(int userID, DateTime date)
        {
            var tasks = _taskRepository.GetOngoingTasksInDay(userID, date);
            return Ok(tasks);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateTask([FromQuery] int userID, [FromQuery] int roleID,
            [FromQuery]int? sectionID, [FromBody] TaskDto taskCreate)
        {
            ...
            else if (taskCreate.TaskTo == null)
            {
